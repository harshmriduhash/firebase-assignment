...
var appendTable = function(){
	...
	if(highScoresArray.length < 10){
		for(i=0; i<appendTable.length; i++){
...
$('#cirnoClick').mousedown(function(){
	if(!gameStart && !gameStarting){
	...
	else if(!gameStart && gameStarting && !readyP1 && !singlePlayer){
		if(clickToPlay>0){
		...
		clickCountP2 += 1;
		$('#clickCountDisplay').text('Clicks: ' +clickCountP2);
...
var scoreDisplay = function() {
	...
	if(clickCountP2>clickCountP1){
		...
		$('#cirnoClickOverlay').append("<div class='animated bounceIn'>Tied?!?!<div id='opponentScore'>Opponent: "+clickCountP1+"</div></div>");	
		setTimeout(newGame, 5000);
	}
	else{
		defeatSound.play();
		$('#cirnoClickOverlay').append("<div class='animated bounceIn'>Defeat!<div id='opponentScore'>Opponent: "+clickCountP1+"</div></div>");
		setTimeout(newGame, 5000);
	}
}
